...
import { makeSchema, type SchemaResult } from "postgraphile/graphile-build";
...
const getAllFixtures = (schema: string) =>
  existsSync(getFixturesDir(schema))
    ? [...Deno.readDirSync(getFixturesDir(schema))].map((f) => f.name).sort()
    : [];

const readSchema = async (schema: string, file?: string) =>
  Deno.readTextFile(getSchemaPath(schema, file));

const readFixture = async (schema: string, fixture: string) =>
  Deno.readTextFile(resolve(getFixturesDir(schema), fixture));

const getSqlSchemas = () =>
  [...Deno.readDirSync(schemasDir)].map((f) => f.name).sort();
          ...
          fn: async (t) => {
            ...
            >(async (pgClient) => {
              ...
              try {
                args.contextValue = {
                  pgSettings:
                    (args.contextValue as { pgSettings: unknown }).pgSettings,
